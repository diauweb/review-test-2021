---
author: 忘怀, Suyun514, mgt, Yanhao, fourdim, NachtgeistW
title: LaTeX 入门
tags:
  - tools
...
每一个参考文献先声名它的文献类型（reference type）。示例中使用的是 @article，其他的类型包括 @book，@incollection 用于引用一本书的中的章节，@inproceedings 用于引用会议论文。可以 [在此](http://en.wikibooks.org/wiki/LaTeX/Bibliography\_Management) 查看更多支持的类型。
