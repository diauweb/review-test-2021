---
author: Lemon, henry_23, Camber, Ir1d
title: 指针
tags:
  - lang
...
:::details[指针变量的大小]{.note open="true"}
...
:::details[列表初始化]{.note open="true"}
`{}` 运算符可以用来初始化没有构造函数的结构。除此之外，使用 `{}` 运算符可以使得变量的初始化形式变得统一。详见「[list initialization (since C++11)](https://en.cppreference.com/w/cpp/language/list\_initialization)」。
:::
...
:::details[二维数组的存储方式]{.note open="true"}
...
:::details[描述数组的维度]{.note open="true"}
...
:::details[数组在物理层面上的线性存储]{.note open="true"}
