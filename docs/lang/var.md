---
author: Camber, Friendseeker, mgt, Shuhao, orzAtalod, XTh3G4p, Xeonacid,
  Margatroid, Ir1d
title: 变量
tags:
  - lang
...
:::details[`int` 类型的大小]{.note open="true"}
...
:::details[等价的类型表述]{.note open="true"}
...
*   ‬如果目标类型为位宽为 $x$ 的带符号整数类型，则 **一般情况下**，转换结果可以认为是原值 $\bmod 2^x$ 后的结果。[^note1]例如将 `unsigned int` 类型的值 $4\ 294\ 967\ 295$（二进制表示为 $1111\ 1111\ 1111\ 1111\ 1111\ 1111\ 1111\ 1111$）转换为 `short` 类型，其值为 $-1$（二进制表示为 $1111\ 1111\ 1111\ 1111$）。
...
[^note1]: 自 C++20 起生效。C++20 前结果是实现定义的。详见 [整型转换 - cppreference](https://zh.cppreference.com/w/cpp/language/implicit\_conversion#.E6.95.B4.E5.9E.8B.E8.BD.AC.E6.8D.A2)。
