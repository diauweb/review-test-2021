---
author: mgt, Chrogeek, Lans1ot, Ir1d
title: 类
tags:
  - lang
...
:::details[关于友元以及派生类的基本概念]{.note open="false"}
...
:::details[常见成员函数举例]{.note open="false"}
```cpp
vector.push_back();
set.insert();
queue.empty();
...
:::details[可以被重载的运算符]{.note open="false"}
```cpp
= +-* / = % += -= *= /= %= <> == != <= >= & | !^~ &= |= ^=
    //----------
    << <<= >> >>= ++--&& || [](),
...
:::details[关于定义(或重载)构造函数]{.note open="false"}
...
:::details[关于隐式类型转换]{.note open="false"}
有时候会写出如下的代码
...
3.  [cppreference default\_constructor](https://zh.cppreference.com/w/cpp/language/default\_constructor)
...
9.  [cppreference value initialization](https://zh.cppreference.com/w/cpp/language/value\_initialization)
