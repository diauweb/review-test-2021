---
author: Danni, mgt, kenlig, Ahacad, schtonn, partychicken, Sshwy, ranwen,
  H-J-Granger, minghao, Zihua, Chaigidel, Chrogeek, hly1204, Shuhao, Early,
  Allenyou, 心旷神怡, abc1763613206, XTh3G4p, ouuan, Ir1d, AngelKitty, Henry,
  TrisolarisHD
title: 快速傅里叶变换
tags:
  - math
...
:::details[为什么用 $n+1$ 个点就能唯一地表示这个函数]{.note open="true"}
...
:::details[递归版 FFT]{.note open="true"}
```cpp
#include <cmath>
#include <complex>
...
:::details[位逆序变换实现（$O(n\log n)$）]{.note open="true"}
```cpp
/*
...
:::details[位逆序变换实现（$O(n)$）]{.note open="true"}
```cpp
// 同样需要保证 len 是 2 的幂
// 记 rev[i] 为 i 翻转后的值
void change(Complex y[], int len) {
...
:::details[非递归版 FFT（对应方法一）]{.note open="true"}
```cpp
/*
 * 做 FFT
 * len 必须是 2^k 形式
...
:::details[非递归版 FFT（对应方法二）]{.note open="true"}
```cpp
/*
 * 做 FFT
 * len 必须是 2^k 形式
...
:::details[FFT 模板（ [HDU 1402](http://acm.hdu.edu.cn/showproblem.php?pid=1402) ）]{open="false"}
```cpp
