---
author: kenlig, nalemy, XTh3G4p, Ren, yzxoi, mgt, sshwy, Xiaodai, Chrogeek,
  Henry-ZHR, Haoshen, ouuan, Ir1d, zhouyuyang2002, 心旷神怡, partychicken, Shuhao,
  Garbage-Only-One, abc1763613206, 北落师门, orzcyand1317, kawa-yoiko, Xeonacid,
  Margatroid, Billchenchina, ShadowsEpic, ddjxd, stevebraveman, HeRaNO
title: 最小生成树
tags:
  - graph
...
下面通过一张动态图来举一个例子（图源自 [维基百科](https://en.wikipedia.org/wiki/Bor%C5%AFvka%27s\_algorithm)）：

![eg](./images/mst-1.apng)

当原图连通时，每次迭代连通块数量至少减半，算法只会迭代不超过 $O(\log V)$ 次，而原图不连通时相当于多个子问题，因此算法复杂度是 $O(E\log V)$ 的。给出算法的伪代码：（修改自 [维基百科](https://en.wikipedia.org/wiki/Bor%C5%AFvka%27s\_algorithm)）
...
:::details[例题：[POJ 1679](http://poj.org/problem?id=1679)]{.note open="false"}
```cpp
...
:::details[POJ 2395 Out of Hay]{.note open="true"}
...
:::details[[「LOJ 137」最小瓶颈路 加强版](https://loj.ac/problem/137)]{.note open="false"}
```cpp
  --8<-- "docs/graph/code/mst/mst_2.cpp"
```
:::

:::details[[NOI 2018 归程](https://uoj.ac/problem/393)]{.note open="false"}
首先预处理出来每一个点到根节点的最短路。
